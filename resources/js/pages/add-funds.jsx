...
import ReportProblem from "../AccountComponents/ReportProblem";
import { PaymentsDrops } from "../BookingComponents/PaymentComp";
import { CheckBtn } from "../SearchBar";
import { close, masterCard } from "../../consonants";

function WalletAddFundPage() {
  const [confirm, setconfirm] = useState(false);

  return confirm ? <AddedPage /> : <AddFundForm setconfirm={setconfirm} />;
}

export default WalletAddFundPage;

const AddFundForm = ({ setconfirm }) => {
  return (
...
const AddedPage = () => {
  ...
  return (
    <div className="flex flex-col pt-[78px] max-w-[619px] w-full pb-10">
      <AnimatePresence>{addedShow && <AddedFund setaddedShow={setaddedShow} />}</AnimatePresence>

      <div className="flex items-center gap-[10px]">
        <AmountShower text="Current Wallet Balance" amount="230,000.00" black={false} />
      ...
      <div className="flex flex-col mt-[27px]">
        <p className="text-[18px] font-[500] leading-[23px] -text--primary-black">Select other methods</p>
        <PaymentsDrops
...
// Other subcomponents: PrevPayCard, SpanHidden, AddedFund, AmountShower
// Keep exactly same as your original code, just make sure imports are relative paths
